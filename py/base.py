...
from pyDatabases import adj, adjMultiIndex, noneInit
...
def fIndexVariable(variableName, v, btype = 'v'):
	return v.set_axis(fIndex(variableName, pyDatabases.getIndex(v), btype = btype)) if isinstance(v, pd.Series) else pd.Series(v, index = fIndex(variableName, None, btype=btype), dtype= np.float64)
...
def stackIndex(ite,names):
	return pd.MultiIndex.from_tuples(np.hstack([pyDatabases.getIndex(f).values for f in ite]), names = names)
