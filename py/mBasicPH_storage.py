...
class mSimple(modelShell):
	...
	def globalDomains(self):
		return {'Generation_E': pydb.cartesianProductIndex([subsetIdsTech(self.db['id2g'], self.modelTech_E, self.db), self.db['h']]), 
				...
				'discharge_H' : adj.rc_pd(pydb.cartesianProductIndex([subsetIdsTech(self.db['id2g'], 'HS', self.db), self.db['h']]), self.db['sCap']),
				'charge_H'	: adj.rc_pd(pydb.cartesianProductIndex([subsetIdsTech(self.db['id2g'], 'HS', self.db), self.db['h']]), self.db['sCap']),
				'stored_H'	: adj.rc_pd(pydb.cartesianProductIndex([subsetIdsTech(self.db['id2g'], 'HS', self.db), self.db['h']]), self.db['sCap']),
				...
				'LawOfMotion_H': pydb.cartesianProductIndex([adj.rc_AdjPd(getTechs('HS',self.db), alias = {'id':'id_alias'}), self.db['h_alias']])}
