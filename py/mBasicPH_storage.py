...
class mSimple(modelShell):
	...
	def globalDomains(self):
		return {'Generation_E': pyDatabases.cartesianProductIndex([subsetIdsTech(self.db['id2g'], self.modelTech_E, self.db), self.db['h']]), 
				...
				'discharge_H' : adj.rc_pd(pyDatabases.cartesianProductIndex([subsetIdsTech(self.db['id2g'], 'HS', self.db), self.db['h']]), self.db['sCap']),
				'charge_H'	: adj.rc_pd(pyDatabases.cartesianProductIndex([subsetIdsTech(self.db['id2g'], 'HS', self.db), self.db['h']]), self.db['sCap']),
				'stored_H'	: adj.rc_pd(pyDatabases.cartesianProductIndex([subsetIdsTech(self.db['id2g'], 'HS', self.db), self.db['h']]), self.db['sCap']),
				...
				'LawOfMotion_H': pyDatabases.cartesianProductIndex([adj.rc_AdjPd(getTechs('HS',self.db), alias = {'id':'id_alias'}), self.db['h_alias']])}
	...
	def A_ub(self):
		return [{'constrName': 'equilibrium_E', 'varName': 'Generation_E', 'value': appIndexWithCopySeries(pd.Series(-1, index = self.globalDomains['Generation_E']), ['g','h'],['g_alias2','h_alias'])},
				...
				{'constrName': 'equilibrium_H', 'varName': 'discharge_H', 'value': appIndexWithCopySeries(pd.Series(-1, index = self.globalDomains['discharge_H']), ['g','h'], ['g_alias2','h_alias'])},
				{'constrName': 'equilibrium_H', 'varName': 'charge_H', 'value': appIndexWithCopySeries(pd.Series(1, index = self.globalDomains['charge_H']), ['g','h'], ['g_alias2','h_alias'])},
				{'constrName': 'LawOfMotion_H', 'varName': 'stored_H', 'value': [appIndexWithCopySeries(pd.Series(1, index = self.globalDomains['stored_H']), ['id','h'], ['id_alias','h_alias']),
																				 rollLevelS(appIndexWithCopySeries(adjMultiIndex.bc(self.db['selfDischarge']-1, self.globalDomains['stored_H']), ['id','h'], ['id_alias','h_alias']), 'h',1)]},
				{'constrName': 'LawOfMotion_H', 'varName': 'discharge_H', 'value': appIndexWithCopySeries(adjMultiIndex.bc(1/self.db['effD'], self.globalDomains['stored_H']), ['id','h'], ['id_alias','h_alias']), 'conditions': getTechs('HS',self.db)},
				{'constrName': 'LawOfMotion_H', 'varName': 'charge_H', 'value': appIndexWithCopySeries(adjMultiIndex.bc(-self.db['effC'] , self.globalDomains['stored_H']), ['id','h'], ['id_alias','h_alias']), 'conditions': getTechs('HS',self.db)}]
