...
def reorderStd(v, order=None):
	return v.reorder_levels(noneInit(order, sorted(pyDatabases.getIndex(v).names))) if isinstance(pyDatabases.getIndex(v), pd.MultiIndex) else v
...
def fIndexVariable(variableName, v, btype = 'v'):
	return v.set_axis(fIndex(variableName, pyDatabases.getIndex(v), btype = btype))
...
def sparseBroadcast(x,y, fill_value=0):
	...
	if pyDatabases.getDomains(y):
		if not pyDatabases.getDomains(x):
			return sparseSeries(x, index = pyDatabases.getIndex(y), fill_value = fill_value)
		elif set(pyDatabases.getDomains(x)).intersection(set(pyDatabases.getDomains(y))):
			return x.add(sparseSeries(0, index = pyDatabases.getIndex(y)), fill_value = fill_value)
		else:
			return sparseSeries(0, index = pyDatabases.cartesianProductIndex([pyDatabases.getIndex(x), pyDatabases.getIndex(y)])).add(x, fill_value=fill_value)
	else:
		return x
